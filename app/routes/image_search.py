...
def search_images(unsplash_key):
    ...
    per_page = min(int(request.args.get('per_page', 12)), 30)  # Limiter à 30 résultats max par page
    ...
    try:
        ...
        for photo in data.get('results', []):
            results.append({
                ...
                'user': {
                    ...
                    'links': {
                        'html': photo.get('user', {}).get('links', {}).get('html', '#')
                    }
                },
                'links': {
        ...
        return jsonify({
            'error': f'Erreur lors de la recherche d\'images: {str(e)}'
        }), 500
